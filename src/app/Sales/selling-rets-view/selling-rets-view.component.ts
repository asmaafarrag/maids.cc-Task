...
@Component({
  selector: 'app-selling-rets-view',
  templateUrl: './selling-rets-view.component.html',
  styleUrls: ['./selling-rets-view.component.css']
})
export class SellingRetsViewComponent implements OnInit {
  ...
  ngOnInit() {
    let pageNo = localStorage.getItem("sellingretspage")
    this.page=+pageNo
  ...
  openForEdit(saleInvId: number) {
    this.router.navigate(['/SellingRets/edit/' + saleInvId]);
  }
  ...
  handlePageChange(event) {
    this.page = event;
    localStorage.setItem( "sellingretspage", event)
    this.getSellingRets();
  }
