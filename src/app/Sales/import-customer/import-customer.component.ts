...
import { CustomerExcelModel } from 'src/app/shared/Models/customer.model';
...
export class ImportCustomerComponent implements OnInit {
  ...
  // constructor(private ItemServ: CustomerService, private router: Router, private toastr: ToastrService, private currentRoute: ActivatedRoute
  ...
  //     this.ItemServ.getCustomersList(params).subscribe(res => {
  ...
  //   this.router.navigate(['/Customers/edit/' + itemId]);
  // }

  // onOrderDelete(ItemIndex: number, ItemId: number) {
  //   if (confirm("هل انت متأكد من حذف هذا العميل")) {
  //     this.ItemServ.deleteCustomer(ItemId).subscribe(
  //       res => {
  //         this.showDeleted();
  ...
  // showDeleted() {
  //   this.toastr.info('تم حذف العميل', 'العملاء');
  // }
  ...
  //   {
  //     this.ExcelServ.SendFileCust(this.ItemList).subscribe(
  //       res => {
  //         this.showSuccess();
  ...
  //       /* save data */
  //       data = XLSX.utils.sheet_to_json(ws,{header:['CustomerName','Tele' , 'tele2' , 'mob1', 'mob2', 'RegistrationNumber' , 'CompanyTypeCode', 'CountryCode', 'GovernateId',  'RegionCityId', 'street', 'buildingNumber', 'postalCode','floor','room' , 'landmark', 'additionalInformation']});
  //       this.ItemList=data;
