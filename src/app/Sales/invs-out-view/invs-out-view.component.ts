...
@Component({
  selector: 'app-invs-out-view',
  templateUrl: './invs-out-view.component.html',
  styleUrls: ['./invs-out-view.component.css']
})
export class InvsOutViewComponent implements OnInit {
  ...
  ngOnInit(): void {
    let pageNo = localStorage.getItem("invsoutpage")
    this.page=+pageNo
  ...
  getInvsOut() {
    ...
    if (this.UserType.toUpperCase() == 'ADMIN') {
      this.InvServ.getInvsOut(params).subscribe(res => {
      ...
      this.InvServ.getInvsOutByUser(this.UserID, params).subscribe(res => {
        const { TotalRecords, Data } = res;
        this.SalesInvList = Data;
        this.count = TotalRecords;
      },
  ...
  openForEdit(saleInvId: number) {
    this.router.navigate(['/InvOut/edit/' + saleInvId]);
  }
  ...
  showDeleted() {
    this.toastr.info('تم حذف السند', 'سندات الصرف');
  }
  ...
  handlePageChange(event) {
    this.page = event;
    localStorage.setItem( "invsoutpage", event)
    this.getInvsOut();
  }
  ...
  onInvOutDelete(InvIndex: number, InvId: number) {
    if (confirm("هل انت متأكد من حذف هذا السند")) {
      this.InvServ.deleteSaleInv(InvId).subscribe(
        res => {
          this.showDeleted();
